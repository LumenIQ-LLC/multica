...
func (b *codexBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	// The public handle always resolves the current attempt under this lock.
	// A retry transition takes the write lock, so it cannot race a control into
	// the discarded process; controls during the transition fail closed.
	...
	go func() {
		...
		for attempt := 1; attempt <= 2; attempt++ {
			if attempt > 1 {
				// Do not let the stable Session.Control closure target attempt 1
				// while attempt 2 is being created.
...
var ErrControlConflict = errors.New("control request id conflicts with its original request")
...
func (c *codexClient) control(ctx context.Context, request ControlRequest) (ControlResult, error) {
	...
	if existing := c.controlRequests[request.RequestID]; existing != nil {
		if existing.fingerprint != fingerprint {
			c.controlMu.Unlock()
			return ControlResult{}, ErrControlConflict
		}
		c.controlMu.Unlock()
		select {
		case <-existing.done:
...
func (c *codexClient) performControl(ctx context.Context, request ControlRequest, threadID, turnID string) (ControlResult, error) {
	...
	select {
	case event := <-waiter:
		if method == "turn/interrupt" && event.status != "interrupted" {
