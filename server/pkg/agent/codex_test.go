...
func TestCodexControlRejectsIdentityAndConflictingRequestIDWithoutWrite(t *testing.T) {
	...
	if _, err := c.control(context.Background(), ControlRequest{RequestID: "one", Operation: ControlCheckpoint, Instruction: "other", ExpectedProviderSessionID: "thread-1", ExpectedProviderTurnID: "turn-1"}); !errors.Is(err, ErrControlRequestConflict) {
